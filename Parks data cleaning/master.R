...
source('~/Documents/REU/Coexistence Spatial Storage/Bimler/data_prep.R')
source('~/Documents/REU/Coexistence Spatial Storage/Bimler/simul_data.R')

# load simulated data 
set.seed(54)
simdat <- simul_data(S=3, T=3, p=0.4)   # p = proportion of interactions which are NOT observed 
#simdat<- read.csv("bimler_sem.csv")
df <- simdat[[1]]
sim_gamma <- simdat[[2]]
...
# Run the model! 
stan.seed <- 1234
fit <- stan(file = '~/Documents/REU/Coexistence Spatial Storage/Bimler/joint_model.stan', 
