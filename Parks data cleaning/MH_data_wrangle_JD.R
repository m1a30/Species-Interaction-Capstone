...
# !! Placeholder estimate of 10 fruits / infl for PLEC & GILCAP ----
  ## updating GILCAP to be 25 fruits / infl
focals <- filter(focals, species %in% sp2)  %>% rowwise() %>% mutate(
  ...
  fruits = ifelse(species == "GILCAP", inflor * 25, fruits)
...
dat_all <- dat_all %>% mutate(
  seeds = case_when(species=="COLLOM" ~ fruits * 3,
                    ...
                    species=="CLAPUR" ~ fruits * 54, # placeholder
...
write.table(dat, "data_cleaned/updated_dat.csv", sep=',', row.names=FALSE, eol = "\r")
write.table(sem_dat, "data_cleaned/updated_sem_dat.csv", sep=',', row.names=FALSE, eol = "\r")
write.table(wir_dat, "data_cleaned/updated_wir_dat.csv", sep=',', row.names=FALSE, eol = "\r")
write.table(br_dat, "data_cleaned/updated_br_dat.csv", sep=',', row.names=FALSE, eol = "\r")
write.table(rf_dat, "data_cleaned/updated_rf_dat.csv", sep=',', row.names=FALSE, eol = "\r")
